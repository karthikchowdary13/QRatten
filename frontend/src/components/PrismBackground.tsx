...
const PrismBackground: React.FC<PrismBackgroundProps> = ({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      iResBuf.set([gl.drawingBufferWidth, gl.drawingBufferHeight]);
      offsetPxBuf.set([offX * dpr, offY * dpr]);
    ...
    const setMat3FromEuler = (y: number, p: number, r: number, out: Float32Array) => {
    ...
    const render = (t: number) => {
      ...
      if (animationType === '3drotate') {
        const tS = time * TS;
        setMat3FromEuler(tS * 0.5, Math.sin(tS * 0.4) * 0.6, Math.sin(tS * 0.3) * 0.5, rotBuf);
